...
- **Vercel Cron is the scheduler path** now that FlowCRM is on Pro, but a cron route has no user session: it needs a server-side Supabase identity. Decision pending with Rachel (she would paste `SUPABASE_SERVICE_ROLE_KEY` into Vercel for that route only; never into files or chat).
